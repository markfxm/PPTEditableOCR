...
- 每页会保存质量清单；未变化且已确认的页面会复用本地修复结果，减少重复导出时间
- 本地修复检测到残影、模糊或复杂背景时，会显示原图与本地结果供用户确认；仅在用户确认后才会对这些页调用 OpenAI 图像编辑
...
11. 如检测到可能的残影、模糊或复杂背景，程序会显示问题页的原图和本地修复预览。可接受本地结果，或输入一次性 OpenAI API Key 仅修复这些页；API Key 不会保存到设置、缓存或日志。
...
- 图片资产使用精细 alpha 提取，底图擦除使用更宽的独立擦除蒙版；导出的透明 PNG 会尝试去除原背景带来的边缘颜色。
- 本地质量缓存保存在项目工作目录的 `quality/page_XXX/quality_manifest.json`。源页、OCR/图片蒙版或修复设置变化后，对应页会自动重新处理。
